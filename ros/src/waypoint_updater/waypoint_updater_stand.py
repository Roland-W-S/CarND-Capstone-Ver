...
LOOKAHEAD_WPS = 50 # Number of waypoints to publish. Set down to 150, was 200
...
class WaypointUpdater(object):	# define class
	...
	# Publishers
        self.final_waypoints_pub = rospy.Publisher('final_waypoints', Lane, queue_size=2)	# 3 enough?
    ...
    def __decelerate(self, waypoints, idx):
        ...
        for i, wp in enumerate(waypoints):  # add numbers
            p = Waypoint()
            p.pose = wp.pose
            stop_idx = max(self.__stopline_wp_idx - idx - 4, 0)	# stop before stop line
            dist = self.distance(waypoints, i, stop_idx)	# calculate distance to decrease velocity proportional to distance
            vel = min(dist / 2, wp.twist.twist.linear.x)	# velocity <= distance, <= current velocity
            if vel < 1.:
                vel = 0.0
